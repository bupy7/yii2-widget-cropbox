...
e(t.croppedContainer).append(n),e(t.croppedDataInput).val(JSON.stringify(c.cropbox("getData")))}),c.on("cb:reset",n),c.on("cb:loaded",n),a(),c.on("cb:disabledCtrls",a),c.on("cb:enabledCtrls",function(){e(t.btnScaleIn).removeAttr("disabled"),e(t.btnScaleOut).removeAttr("disabled"),e(t.btnCrop).removeAttr("disabled")}),c.on("cb:cropped",function(){r()}),e(t.fileInput).on("change",function(){r(!0)}),c.on("cb:reset",function(){e(t.messageContainer).hide()})}e.fn.cropbox=function(n){if("string"==typeof n){var a=n.replace(/^[_]*/,"")
